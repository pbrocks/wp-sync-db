...
class WPSDB extends WPSDB_Base {
	...
	function options_page() {
		...
		<div class="wrap wpsdb">
			...
			<div id="icon-tools" class="icon32"><br /></div><h2>WP Sync DB</h2>
	...
	function network_admin_menu() {
		$hook_suffix = add_submenu_page( 'settings.php', 'WP Sync DB', 'WP Sync DB', 'manage_network_options', 'wp-sync-db', array( $this, 'options_page' ) );
		$this->after_admin_menu( $hook_suffix );
	}

	function admin_menu() {
		$hook_suffix = add_management_page( 'WP Sync DB', 'WP Sync DB', 'export', 'wp-sync-db', array( $this, 'options_page' ) );
		$this->after_admin_menu( $hook_suffix );
	}
	...
	function load_assets() {
		...
		wp_localize_script( 'wp-sync-db-script', 'wpsdb_i10n', array(
			...
			'migrate_db_save'						=> __( "WP Sync DB & Save", 'wp-sync-db' ),
			'migrate_db'							=> __( "WP Sync DB", 'wp-sync-db' ),
