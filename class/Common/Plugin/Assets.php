...
class  Assets
{
    ...
    function load_assets()
    {
        ...
        wp_localize_script('wp-migrate-db-pro-script-v2',
            ...
            apply_filters('wpmdb_js_strings', array(
                ...
                'connection_info_key_invalid'           => __('The secret key on the second line appears to be invalid. It should be a 40 character string that consists of letters, numbers and special characters only.', 'wp-sync-db'),
                ...
                'connection_info_local_key'             => __('Looks like your remote secret key is the same as the secret key for this site. To fix this, go to the <a href="#settings">Settings tab</a> and click "Reset Secret Key"', 'wp-sync-db'),
                'migration_cancelled'                   => _x('Migration cancelled', 'The migration has been cancelled', 'wp-sync-db'),
                'migration_cancelled_success'           => __('The migration has been stopped and all temporary files and data have been cleaned up.', 'wp-sync-db'),
                'welcome_title'                         => __('Welcome to WP Migrate! &#127881;', 'wp-sync-db'),
                'welcome_text'                          => __('Hey, this is the first time activating your license, nice! Your migrations are about to get awesome! If you haven’t already, you should check out our <a href="%1$s" target="_blank">Quick Start Guide</a> and <a href="%2$s" target="_blank">Videos</a>. If you run into any trouble at all, use the <strong>Help tab</strong> above to submit a support request.', 'wp-sync-db'),
                'invalid_sql_file'                      => __('The selected file does not have a recognized file type. Please upload a valid SQL file to continue.', 'wp-sync-db'),
                'please_select_sql_file'                => __('Please select an SQL export file above to continue.', 'wp-sync-db'),
                'importing_file_to_db'                  => __('Importing data from %s', 'wp-sync-db'),
    ...
    public function wpmdb_enqueue_assets()
    {
        ...
        if ($this->util->isMDBPage() && Util::is_wp_compatible()) {
            // $plugin_slug = basename(WPMDB_FILE);
            $plugin_slug = basename(dirname(dirname(dirname(__DIR__))));
            $assets_path = apply_filters('wpmdb_frontend_assets_path', WP_PLUGIN_DIR . '/' . $plugin_slug . '/frontend');
            \ReactWPScripts\enqueue_assets(
                $assets_path,
                [
                    'base_url' => plugins_url($plugin_slug . '/frontend'),
                    'key'      => 'mdb',
                ]
            );
